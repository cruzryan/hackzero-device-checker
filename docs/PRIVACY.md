...
- encryption, screen-lock, auto-update, and endpoint-protection outcomes
  with short remediation codes, and the setting values behind each outcome
  (for example display-off minutes per power source, the password delay,
  update switches, whether Defender or another antivirus is healthy, and the
  malware-definitions version and age), as numbers, true/false values and
  fixed codes only. Product names and raw command output are never sent.
